...
# posição inicial fake do entregador
...
def normalize_route(route: list):
    """
    Converte rota no formato [[lat, lon], [lat, lon], ...]
    para uma lista válida.
    """
    if not isinstance(route, list) or not route:
        raise ValueError("route inválida ou vazia")

    normalized = []
    for i, point in enumerate(route):
        if not isinstance(point, (list, tuple)) or len(point) != 2:
            raise ValueError(f"point inválido em route[{i}]")
        lat, lon = point
...
def process_delivery(message: dict):
    ...
    # route_to_restaurant = normalize_route(message["route_to_pickup"])
    # route_to_client = normalize_route(message["route_to_delivery"])
...
def callback(ch, method, properties, body):
    try:
        ...
        if message_courier_id != COURIER_ID:
            print(
                f"[COURIER {COURIER_ID}] Ignorando pedido de outro courier: "
                f"{message_courier_id}"
            )
            ch.basic_ack(delivery_tag=method.delivery_tag)
            return
...
def main():
    ...
    # registra localização inicial no sistema
    try:
        send_location(COURIER_ID, INITIAL_LAT, INITIAL_LON, 0)
    except Exception as e:
        print(f"Não foi possível registrar localização inicial: {e}")
