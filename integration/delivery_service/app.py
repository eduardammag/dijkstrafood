...
ROUTING_URL = os.getenv("ROUTING_URL", "http://routing-service:8000/rota")
...
def assign_courier_in_api(order_id: int, courier_id: int, route_to_restaurant: list, route_to_client:list):
    response = requests.post(
        ...
        json={"courier_id": courier_id, "route_to_pickup": route_to_restaurant,"route_to_delivery": route_to_client},
        timeout=15
    )
    response.raise_for_status()
    return response.json()
...
def alocar_entrega(data: dict):
    ...
    try:
        ...
        route_to_restaurant = gerar_rota_simples((entregador["lat"], entregador["lon"]),(restaurante["lat"], restaurante["lon"]))
        route_to_client = gerar_rota_simples((restaurante["lat"], restaurante["lon"]),(cliente["lat"], cliente["lon"]))
...
def consume_delivery_dispatch():
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            # coordenadas fake enquanto você ainda não puxar do banco
            restaurante = {
